...
Learning test for non-linear state space system initial state = 0 with noise.
...
noiseless_outputs = np.copy(train_outputs)
train_outputs += np.random.normal(0, 0.1, size=(number_of_sample_train, STATE_SPACE_VECTOR_LENGTH))
...
axs[0, 0].plot(noiseless_outputs[:number_of_sample_test, 0], 'b', label='true noiseless state', linewidth=4)
axs[0, 0].plot(train_outputs[:number_of_sample_test, 0], 'g', label='true state', linewidth=2)
axs[0, 0].plot(predicted_train_outputs[:, 0], '--r', label='predicted state', linewidth=2)
axs[0, 0].set_title('train: x_1')
axs[0, 0].legend()
axs[0, 1].plot(noiseless_outputs[:number_of_sample_test, 1], 'b', label='true noiseless state', linewidth=4)
axs[0, 1].plot(train_outputs[:number_of_sample_test, 1], '--g', label='true state', linewidth=2)
