...
class LinearStateSpaceSystem:
    ...
    def response(self, inputs: np.ndarray, initial_state: np.ndarray):
        ...
        :param inputs: array of u_{k} for k = 1, 2, ..., N - 1. shape=(N, p)
        ...
        number_of_output_samples = inputs.shape[0]
        outputs = np.zeros((number_of_output_samples + 1, self.q))
        ...
        for k in range(number_of_output_samples):
            state_space_sequence[:, k + 1: k + 2] = self.A @ state_space_sequence[:, k: k + 1] + self.B @ inputs[k: k + 1, :]
            # state_space_sequence[:, k + 1: k + 2] += 0.5 * np.tanh(state_space_sequence[:, k: k + 1])
        # TODO: Implement noise 
        outputs[-1, :] = self.C @ state_space_sequence[:, -2: -1]
        self.states = state_space_sequence
        ...
        return {'inputs': inputs, 'outputs': outputs, 'states': self.states}
