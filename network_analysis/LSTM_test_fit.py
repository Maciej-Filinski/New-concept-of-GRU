import datetime
import tensorflow as tf
from tensorflow import keras
from data_generator import ToyProblemRealRoots, ToyProblemComplexRoots, ToyProblemOriginal
import numpy as np
import matplotlib.pyplot as plt
from layers import BasicLSTM

EPOCHS = 500
TRAIN_SAMPLES = 500
TEST_SAMPLES = 100
BATCH_SIZE = 100
STATE_LENGTH = 16
data_generator = ToyProblemComplexRoots(number_of_train_samples=TRAIN_SAMPLES, number_of_test_samples=TEST_SAMPLES,
                                        dataset_name='toy_problem_complex_roots')

dataset = data_generator.load_data()


build_in_model = keras.Sequential()
build_in_model.add(keras.layers.LSTM(STATE_LENGTH, batch_input_shape=(1, 1, 1), stateful=True))
build_in_model.add(keras.layers.Dense(1))
build_in_model.compile(optimizer='adam', loss=tf.keras.losses.MeanSquaredError(),
                       metrics=[tf.keras.metrics.RootMeanSquaredError()])
build_in_model.summary()
initial_weight = build_in_model.get_weights()

cell = BasicLSTM(state_length=STATE_LENGTH)
rnn = keras.layers.RNN(cell, stateful=True)
input_layer = keras.Input(batch_shape=(1, None, 1), ragged=True)
output = rnn(input_layer)
output = keras.layers.Dense(1)(output)
custom_model = keras.models.Model(input_layer, output)
custom_model.compile(optimizer='adam', loss=tf.keras.losses.MeanSquaredError(),
                     metrics=[tf.keras.metrics.RootMeanSquaredError()])
custom_model.summary()
possible = tf.split(initial_weight[1], 4, axis=1)

custom_model.set_weights(initial_weight)

custom_model.reset_states()
build_in_model.reset_states()


history_build_in = []
history_custom = []
x_train = dataset['train']['inputs']
y_train = dataset['train']['outputs']
indices_set = list(range(TRAIN_SAMPLES))
number_of_batch = TRAIN_SAMPLES // BATCH_SIZE
for epoch in range(EPOCHS):
    start_epoch_time = datetime.datetime.now()
    for step in range(number_of_batch):
        start_step_time = datetime.datetime.now()
        batch_shuffle_set = indices_set[step * BATCH_SIZE: (step + 1) * BATCH_SIZE]

        max_value = np.max(batch_shuffle_set)
        build_in_model.reset_states()
        y_predicted = []
        with tf.GradientTape() as tape:
            for i in range(max_value + 1):
                y = build_in_model(x_train[i: i+1, :, :])
                y_predicted.append(y)
            y_predicted = tf.convert_to_tensor(y_predicted)
            y_1 = []
            y_2 = []
            for i in batch_shuffle_set:
                y_1.append(y_train[i, :])
                y_2.append(y_predicted[i, :, :])
            y_1 = tf.convert_to_tensor(y_1)
            y_2 = tf.convert_to_tensor(y_2)
            loss = build_in_model.compiled_loss(y_1, y_2)
        grads = tape.gradient(loss, build_in_model.trainable_weights)
        build_in_model.optimizer.apply_gradients(zip(grads, build_in_model.trainable_weights))

        end_step_time = datetime.datetime.now()
        print(f'epoch:{epoch + 1:3}/{EPOCHS:3}, step:{step + 1:3}/{number_of_batch:3},',
              f'step execution time:{(end_step_time - start_step_time).total_seconds():.2f}s,',
              f'loss:{loss:.8f}')
    build_in_model.reset_states()
    predicted_outputs_build_in = np.zeros((TRAIN_SAMPLES, 1))
    for i in range(TRAIN_SAMPLES):
        predicted_outputs_build_in[i, :] = build_in_model.predict(dataset['train']['inputs'][i: i + 1, :, :])
    history_build_in.append(build_in_model.compiled_loss(tf.convert_to_tensor(predicted_outputs_build_in),
                                                         tf.convert_to_tensor(y_train)))

for epoch in range(EPOCHS):
    start_epoch_time = datetime.datetime.now()
    for step in range(number_of_batch):
        start_step_time = datetime.datetime.now()
        batch_shuffle_set = indices_set[step * BATCH_SIZE: (step + 1) * BATCH_SIZE]

        max_value = np.max(batch_shuffle_set)
        custom_model.reset_states()
        y_predicted = []
        with tf.GradientTape() as tape:
            for i in range(max_value + 1):
                y = custom_model(x_train[i: i+1, :, :])
                y_predicted.append(y)
            y_predicted = tf.convert_to_tensor(y_predicted)
            y_1 = []
            y_2 = []
            for i in batch_shuffle_set:
                y_1.append(y_train[i, :])
                y_2.append(y_predicted[i, :, :])
            y_1 = tf.convert_to_tensor(y_1)
            y_2 = tf.convert_to_tensor(y_2)
            loss = custom_model.compiled_loss(y_1, y_2)
        grads = tape.gradient(loss, custom_model.trainable_weights)
        custom_model.optimizer.apply_gradients(zip(grads, custom_model.trainable_weights))

        end_step_time = datetime.datetime.now()
        print(f'epoch:{epoch + 1:3}/{EPOCHS:3}, step:{step + 1:3}/{number_of_batch:3},',
              f'step execution time:{(end_step_time - start_step_time).total_seconds():.2f}s,',
              f'loss:{loss:.8f}')
    custom_model.reset_states()
    predicted_outputs_custom = np.zeros((TRAIN_SAMPLES, 1))
    for i in range(TRAIN_SAMPLES):
        predicted_outputs_custom[i, :] = custom_model.predict(dataset['train']['inputs'][i: i + 1, :, :])
    history_custom.append(custom_model.compiled_loss(tf.convert_to_tensor(predicted_outputs_custom),
                                                     tf.convert_to_tensor(y_train)))

"""
TEST
"""
build_in_model.reset_states()
predicted_outputs_build_in = np.zeros((TEST_SAMPLES, 1))
for i in range(TEST_SAMPLES):
    predicted_outputs_build_in[i, :] = build_in_model.predict(dataset['test']['inputs'][i: i + 1, :, :])

custom_model.reset_states()
predicted_outputs_custom = np.zeros((TEST_SAMPLES, 1))
for i in range(TEST_SAMPLES):
    predicted_outputs_custom[i, :] = custom_model.predict(dataset['test']['inputs'][i: i + 1, :, :])


fig, axs = plt.subplots(3, 2)
fig.suptitle(f'{TRAIN_SAMPLES=}, {TEST_SAMPLES=}, {EPOCHS=}, LSTM_{STATE_LENGTH=}')
axs[0, 0].plot(dataset['test']['outputs'], label='true outputs')
axs[0, 0].plot(predicted_outputs_build_in, label='predicted outputs')
axs[0, 0].legend()
axs[0, 0].grid()
axs[0, 0].set_title('build-in LSTM')
axs[1, 0].plot(dataset['test']['outputs'], label='true outputs')
axs[1, 0].plot(predicted_outputs_custom, label='predicted outputs')
axs[1, 0].legend()
axs[1, 0].grid()
axs[1, 0].set_title('custom LSTM')
axs[2, 0].plot(predicted_outputs_build_in, label='build-in')
axs[2, 0].plot(predicted_outputs_custom, label='custom')
axs[2, 0].plot(abs(predicted_outputs_build_in - predicted_outputs_custom), label='difference')
axs[2, 0].legend()
axs[2, 0].grid()
axs[2, 0].set_title(f'build-in LSTM vs custom LSTM')

axs[0, 1].plot(history_build_in, label='train loss')
axs[0, 1].legend()
axs[0, 1].grid()
axs[0, 1].set_title('build-in LSTM')
axs[1, 1].plot(history_custom, label='train loss')
axs[1, 1].legend()
axs[1, 1].grid()
axs[1, 1].set_title('custom LSTM')
axs[2, 1].plot(history_custom, label='build-in LSTM train loss')
axs[2, 1].plot(history_build_in, label='custom LSTM train loss')
axs[2, 1].plot(abs(np.array(history_custom) - np.array(history_build_in)), label='difference')
axs[2, 1].set_xlabel('epoch')
axs[2, 1].legend()
axs[2, 1].grid()
axs[2, 1].set_title(f'build-in LSTM vs custom LSTM')
plt.show()
